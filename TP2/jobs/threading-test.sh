...
# ABOUT --------------------------------------------------------------------------------------------
#
# Deucalion job for testing if model performance varies with different numbers of threads and
# NUMA configuration.
#
# Usage: $ ./threading-test.sh
#
# LICENSE ------------------------------------------------------------------------------------------
#
...
#
# General properties
#SBATCH --job-name=threading-test
...
# Directory where to output the threading results to
OUTPUT_DIRECTORY='../results/threading'
...
# Numbers of threads to test
THREADS='12 24 36 44 48'

# Seed to be used for all prompts
SEED=0

# Number of runs per prompt and model (reduced to six to save time)
RUNS=6
...
if [ "$#" = 0 ]; then
    ...
    find '../models' -type f -name "*$QUANTIZATION*" | while IFS= read -r model_file; do
        for threads in $THREADS; do
            # Do not test --numa=isolate. It is too slow.
            for numa in distribute; do
                sbatch "$0" "$model_file" "$threads" "$numa"
            done
        done
    done
else
    ...
    output_file="$OUTPUT_DIRECTORY/$model_name-$quantization-t$threads-$numa.csv"

    # Write output CSV file header
    printf 'COMPILER,MODEL,QUANTIZATION,SEEDING,PROMPT,THREADS,NUMA,RUN,' >  "$output_file"
    ...
    find '../prompts' -type f -name '0' | sort -r | while IFS= read -r prompt_file; do
        ...
        BLIS_NUM_THREADS="$threads"                                        \
            ...
            -t "$threads" --numa "$numa" --no-mmap -m "$model_file" 2>/dev/null &
        ...
        for run in $(seq "$RUNS"); do
            ...
            printf '%s,%s,%s,%s,%s,%s,%s,%s,'            \
                ...
                "$prompt_type" "$threads" "$numa" "$run" >> "$output_file"
