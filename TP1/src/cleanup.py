# ABOUT --------------------------------------------------------------------------------------------
#
# Cleaned up query implementation.
#
# LICENSE ------------------------------------------------------------------------------------------
#
...
class Cleanup(Query):
    ...
    def process_dataset(self, _: Context) -> None:
        ...
        for tag, months in self.tag_months.items():
            ...
            completed = in_months.groupby(F.when(F.col("State") == "COMPLETED", "COMPLETED") \
                                        .otherwise("FAILED")                                 \
                                        .alias("Completed"),
                                        "Cluster")                                           \
                                .count()                                                     \
            ...
            for row in completed:
                if row.Completed == "COMPLETED":
                    self.output_parameters[f"{row.Cluster.lower()}CompletedJobs{tag}"] = row.asDict()["count"]
                else:
                    self.output_parameters[f"{row.Cluster.lower()}FailedJobs{tag}"] = row.asDict()["count"]

            not_local = in_months.filter(F.col("Agency") != "LOCAL")

            clusters_hours = not_local.groupby("Cluster") \
                                    .agg(
                                        F.sum("TotalJobSeconds").alias("SumRes"),
                                        F.count("*").alias("CountRes")
                                    ).collect()
            
            for row in clusters_hours:
                hours[row.asDict()["Cluster"]] = row.asDict()["SumRes"]
                jobs[row.asDict()["Cluster"]] = row.asDict()["CountRes"]
            ...
            ehpc_list = in_months.filter(F.col("Agency") == "EHPC")             \
                                .groupby("Cluster")                             \
                                .agg(
                                    F.count("*").alias("CountRes"),
                                    F.sum("TotalJobSeconds").alias("SumRes"))   \
            ...
            for row in ehpc_list:
                self.output_parameters[f"{row.Cluster.lower()}usedhoursEuroHPC{tag}"] = row.asDict()["SumRes"]   / 3600
                self.output_parameters[f"{row.Cluster.lower()}JobsEuroHPC{tag}"]      = row.asDict()["CountRes"]
