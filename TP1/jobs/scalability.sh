...
# ABOUT --------------------------------------------------------------------------------------------
#
# Deucalion job for performing scalability analyses query implementations.
#
# LICENSE ------------------------------------------------------------------------------------------
#
...
mkdir -p results
for query_class in $SCALABILITY_QUERY_CLASSES; do
    ...
    aded/perf.py                                  \
        -w "$PERF_WARMUP_RUNS"                    \
        -r "$PERF_RUNS"                           \
        -o "results/Scalability$query_class.json" \
        -n "$SCALABILITY_THREADS"                 \
        "$query_class"                            \
        "$DATASET_PATH/$YEAR"
